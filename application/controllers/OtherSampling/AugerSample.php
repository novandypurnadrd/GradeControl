...
class AugerSample extends CI_Controller {

	public function AugerSample(){
		parent::__construct();
	...
	public function InputAugerSample()
	{
    	if ($this->session->userdata('GradeControl')) {
    		...
    		$this->OtherSampling_model->InputAugerSample($AugerSample);
	...
	public function DeleteAugerSample($id)
	{
		if ($this->session->userdata('GradeControl')) {
			$this->OtherSampling_model->DeleteAugerSample($id);
			redirect('OtherSampling/AugerSample');
		}
		else {
			redirect(base_url());
